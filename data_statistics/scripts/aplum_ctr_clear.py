...
# 获取ctr索引列表
def getIndexList():
    data = es.indices.get(index='aplum_ctr-*').keys()
    indexlist = list(data)
    return indexlist
...
if __name__ == '__main__':
    try:
        ...
        print('当前时间: {} aplum_ctr es磁盘使用率为{}'.format(timestr,percent))
        ...
        print("delete aplum_ctr accurs error, the error:%s" % str(msg))
        userlist = ['liwenlong@aplum.com', 'suzerui@aplum.com']
        err = "清理aplum_ctr历史数据出错"
        alarm(userlist,err)
