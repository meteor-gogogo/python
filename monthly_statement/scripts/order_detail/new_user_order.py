...
def get_data(url, date_tmp, aplum_cursor, start_date_tmp, end_date_tmp):
    sql = "select \
            sum(OrderItem_RealpayPrice)/sum(OrderItem_DiscountPrice) as one, \
            sum(OrderItem_DiscountPrice) as two, \
            sum(OrderItem_RealpayPrice) as three, \
            count(distinct orderid) as four, \
            sum(OrderItem_RealpayPrice)/count(distinct orderid) as five \
            from events \
            ...
            from events \
            where event='GeneralOpenApp' \
            and date>='{2}' \
            and date<'{3}' \
            and IsFirstTime=1 \
            group by user_id)".format(start_date_tmp, end_date_tmp, start_date_tmp, end_date_tmp)
        ...
        if len(datastr) == 0:
            ...
            for data in dataarr:
                try:
                    ...
                    if str(datajson) == '{}':
                        continue
                    one = round(float(datajson['one']), 2)
                    two = round(float(datajson['two']), 2)
                    three = round(float(datajson['three']), 2)
                    four = int(datajson['four'])
                    five = round(float(datajson['five']), 2)
...
def get_all_data(url, date_tmp, aplum_cursor, result_dict, start_date_tmp, end_date_tmp, start_timestamp, end_timestamp):
    # 当前月份
    ...
    one, two, three, four, five = \
        get_data(url, date_tmp, aplum_cursor, start_date_tmp, end_date_tmp)
    #
    # # 订单数
    ...
    result_dict['1'].append(one)
    result_dict['2'].append(two)
    result_dict['3'].append(three)
    result_dict['4'].append(four)
    result_dict['5'].append(five)
...
if __name__ == '__main__':
    ...
    for date_tmp in date_list:
        ...
        result_dict = get_all_data(url, date_tmp, aplum_cursor, result_dict, start_date_tmp, end_date_tmp, start_timestamp,
                                   end_timestamp)
        # print(result_dict)
    ...
    today = date.today()
    writer = pd.ExcelWriter('/home/aplum/work_lh/new_user_order_{0}.xlsx'.format(today))
